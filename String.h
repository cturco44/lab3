...
String& String::erase(size_t pos, size_t len) {
    ...
    //swap if they are letters
    *ptr1 = *ptr2;
    //keep swapping letters
    while(*ptr2 != a_null_byte) {
        ++ptr1;
        ++ptr2;
        *ptr1 = *ptr2;
    }
    //swap null terminator
    *ptr1 = *ptr2;
    sz = sz - len;
...
String& String::insert(size_t pos, const String& str) {
    ...
    for(; ptr >= end; --ptr) {
        char* copy_ptr = ptr;
        copy_ptr += insert_size;
        *copy_ptr = *ptr;
    }
    int j = 0;
    for(size_t i = pos; i < pos + insert_size; ++i) {
        cstr[i] = copy_str[j];
        ++j;
    }
    sz += insert_size;
...
String& String::replace(size_t pos, size_t len, const String& str) {
    ...
    String copy = str;
    size_t copy_length = str.sz;
    this->insert(pos, copy);
    size_t start_delete = pos + copy_length;
    this->erase(start_delete, len);
...
size_t String::find_first_of(const String& str, size_t pos) {
    ...
    return npos; // GET RID OF THIS LINE WHEN YOU BEGIN IMPLEMENTING FIND_FIRST_OF (this is for compile purposes only)
}
...
size_t String::find_last_of(const String& str, size_t pos) {
    ...
    return npos; // GET RID OF THIS LINE WHEN YOU BEGIN IMPLEMENTING FIND_LAST_OF (this is for compile purposes only)
}
